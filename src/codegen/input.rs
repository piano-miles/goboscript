...
where T: Write + Seek
{
    pub fn input(&mut self, s: S, d: D, name: &str, expr: &Expr, this_id: NodeID) -> io::Result<()> {
    ...
    fn _input(
        ...
        match expr {
            ...
            Expr::Name(name) => self.name_input(s, d, input_name, name),
        ...
        match value {
            ...
            Value::String(string_value) => {
                let color = ["COLOR", "COLOR2"]
                    .contains(&name)
                    .then(|| csscolorparser::parse(string_value).ok().filter(|color| color.a == 1.0))
                    .flatten();
                if name == "BROADCAST_INPUT" {
                    write!(self, "[1,[11,{},{}]]", json!(**string_value), json!(**string_value))
    ...
    fn name_input(&mut self, s: S, d: D, _input_name: &str, name: &Name) -> io::Result<()> {
    ...
    fn node_input(&mut self, s: S, _d: D, input_name: &str, node_id: NodeID, shadow_id: Option<NodeID>) -> io::Result<()> {
    ...
    fn shadow_input(&mut self, _s: S, input_name: &str, shadow_id: Option<NodeID>) -> io::Result<()> {
