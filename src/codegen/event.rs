...
where T: Write + Seek
{
    ...
    pub fn on_key(&mut self, _s: S, _d: D, _this_id: NodeID, key: &SmolStr, _span: &Span) -> io::Result<()> {
    ...
    pub fn on_backdrop(&mut self, _s: S, _d: D, _this_id: NodeID, backdrop: &SmolStr, _span: &Span) -> io::Result<()> {
        self.single_field("BACKDROP_OPTION", backdrop)?;
        self.end_obj() // node
    }

    pub fn on_loudness_gt(&mut self, s: S, d: D, this_id: NodeID, value: &Rrc<Expr>) -> io::Result<()> {
        self.begin_inputs()?;
    ...
    pub fn on_timer_gt(&mut self, s: S, d: D, this_id: NodeID, value: &Rrc<Expr>) -> io::Result<()> {
        self.begin_inputs()?;
        self.input(s, d, "VALUE", &value.borrow(), this_id)?;
        self.end_obj()?; // inputs
