...
where T: Write + Seek
{
    pub fn repeat(&mut self, s: S, d: D, this_id: NodeID, times: &Rrc<Expr>, body: &[Stmt]) -> io::Result<()> {
    ...
    pub fn forever(&mut self, s: S, d: D, this_id: NodeID, body: &[Stmt], _span: &Span) -> io::Result<()> {
        let body_id = self.id.new_id();
        self.begin_inputs()?;
        self.substack("SUBSTACK", (!body.is_empty()).then_some(body_id))?;
        self.end_obj()?; // inputs
    ...
    pub fn until(&mut self, s: S, d: D, this_id: NodeID, cond: &Rrc<Expr>, body: &[Stmt]) -> io::Result<()> {
        let cond_id = self.id.new_id();
        let body_id = self.id.new_id();
        self.begin_inputs()?;
        self.input(s, d, "CONDITION", &cond.borrow(), cond_id)?;
    ...
    pub fn set_var(
        ...
        match s.qualify_name(d, name) {
            Some(QualifiedName::Var(qualified_name, _)) => self.single_field_id("VARIABLE", &qualified_name)?,
            Some(QualifiedName::List(..)) => {
                d.report(DiagnosticKind::UnrecognizedVariable(name.basename().clone()), &name.span());
            }
            None => {}
        }
        self.end_obj()?; // node
    ...
    pub fn show(&mut self, s: S, d: D, name: &Name) -> io::Result<()> {
        ...
        match s.qualify_name(d, name) {
            Some(QualifiedName::Var(qualified_name, _)) => self.single_field_id("VARIABLE", &qualified_name)?,
            Some(QualifiedName::List(qualified_name, _)) => self.single_field_id("LIST", &qualified_name)?,
            None => {}
        }
        self.end_obj() // node
    }
    ...
    pub fn add_to_list(&mut self, s: S, d: D, this_id: NodeID, name: &Name, value: &Rrc<Expr>) -> io::Result<()> {
        let value_id = self.id.new_id();
        self.begin_inputs()?;
        ...
        match s.qualify_name(d, name) {
            ...
            Some(QualifiedName::Var(..)) => {
                d.report(DiagnosticKind::UnrecognizedList(name.basename().clone()), &name.span());
            }
            None => {}
        }
        self.end_obj()?; // node
    ...
    pub fn delete_list_index(&mut self, s: S, d: D, this_id: NodeID, name: &Name, index: &Rrc<Expr>) -> io::Result<()> {
    ...
    pub fn block(&mut self, s: S, d: D, this_id: NodeID, block: &Block, span: &Span, args: &Vec<Rrc<Expr>>) -> io::Result<()> {
        if block.args().len() != args.len() {
            d.report(DiagnosticKind::BlockArgsCountMismatch { block: *block, given: args.len() }, span)
        }
        self.begin_inputs()?;
        ...
        for ((&arg_name, arg_value), &arg_id) in block.args().iter().zip(args).zip(&arg_ids) {
            if block.menu().is_some_and(|menu| menu.input == arg_name) {
                if let Expr::Value { value, span: _ } = &*arg_value.borrow() {
                    ...
                    self.input_with_shadow(s, d, arg_name, &arg_value.borrow(), arg_id, menu_id.unwrap())?;
                }
            } else {
        ...
        if menu_is_default {
            ...
            write!(self, r#""{}":[1,{}]"#, block.menu().unwrap().input, menu_id.unwrap())?;
        }
        self.end_obj()?; // inputs
        ...
        if let Some(menu) = block.menu() {
            self.begin_node(Node::new(menu.opcode, menu_id.unwrap()).parent_id(this_id).shadow(true))?;
    ...
    pub fn proc_call(
        ...
        if proc.args.len() != args.len() {
            d.report(DiagnosticKind::ProcArgsCountMismatch { proc: name.clone(), given: args.len() }, span)
        }
                ...
                Type::Struct { name: type_name, span: type_span } => {
                    ...
                    let struct_literal_fields = match arg_value {
                        Expr::StructLiteral {
                            ...
                            for (struct_field, struct_literal_field) in struct_.fields.iter().zip(struct_literal_fields) {
                    ...
                    for (field, struct_literal_field) in struct_.fields.iter().zip(struct_literal_fields) {
                        let qualified_arg_name = qualify_struct_var_name(&field.name, &arg.name);
                        let arg_id = self.id.new_id();
                        self.input(s, d, &qualified_arg_name, &struct_literal_field.value.borrow(), arg_id)?;
        ...
        self.end_obj()?; // inputs
        write!(self, "{}", Mutation::call(proc.name.clone(), &qualified_args, proc.warp))?;
        self.end_obj()?; // node
