...
use super::{node::Node, node_id::NodeID, node_id_factory::NodeIDFactory, turbowarp_config::TurbowarpConfig};
use crate::{
    ast::*,
    blocks::Block,
...
impl<'a> S<'a> {
    pub fn is_name_list(&self, name: &Name) -> bool {
        self.sprite.lists.contains_key(name.basename())
            || self.stage.is_some_and(|stage| stage.lists.contains_key(name.basename()))
    }
    ...
    fn get_var(&self, name: &str) -> Option<&Var> {
        self.sprite.vars.get(name).or_else(|| self.stage.and_then(|stage| stage.vars.get(name)))
    }

    fn get_list(&self, name: &str) -> Option<&List> {
        self.sprite.lists.get(name).or_else(|| self.stage.and_then(|stage| stage.lists.get(name)))
    }

    fn get_struct(&self, name: &str) -> Option<&Struct> {
        self.sprite.structs.get(name).or_else(|| self.stage.and_then(|stage| stage.structs.get(name)))
    }
        ...
        T: FnOnce(SmolStr, Type) -> QualifiedName,
    {
        match type_ {
            ...
            Type::Struct { name: type_name, span: type_span } => match field_name {
                ...
                Some(field_name) => {
                    ...
                    if !struct_.fields.iter().any(|field| field.name == field_name) {
                        ...
                        Some(variant(qualify_struct_var_name(&field_name, &qualified_var_name), type_.clone()))
                    }
                }
            },
        }
    ...
    pub fn qualify_name(&self, d: D, name: &Name) -> Option<QualifiedName> {
        ...
        if let Some(list) = self.get_list(basename) {
            return self.qualify_field(d, &name.span(), list.name.clone(), fieldname, &list.type_, QualifiedName::List);
        }
        if let Some(var) = self.get_local_var(basename) {
            let qualified_var_name = qualify_local_var_name(&self.proc.unwrap().name, &var.name);
            return self.qualify_field(d, &name.span(), qualified_var_name, fieldname, &var.type_, QualifiedName::Var);
        }
        if let Some(var) = self.get_var(basename) {
            return self.qualify_field(d, &name.span(), var.name.clone(), fieldname, &var.type_, QualifiedName::Var);
        }
        d.report(DiagnosticKind::UnrecognizedVariable(basename.clone()), &name.span());
        None
    }
}
...
impl Stmt {
    fn is_terminator(&self) -> bool {
        matches!(
            self,
            Stmt::Forever { .. } | Stmt::Block { block: Block::DeleteThisClone | Block::StopAll | Block::StopThisScript, .. }
        )
    }
...
where T: Write + Seek
{
    ...
    pub fn substack(&mut self, name: &str, this_id: Option<NodeID>) -> io::Result<()> {
        let Some(this_id) = this_id else { return Ok(()) };
    ...
    pub fn project(
        ...
        self.zip.start_file("project.json", SimpleFileOptions::default())?;
        write!(self, "{{")?;
        write!(self, r#""targets":["#)?;
        self.sprite(input, STAGE_NAME, &project.stage, None, config, stage_diagnostics)?;
        ...
        write!(self, r#","agent":"goboscript v{}""#, env!("CARGO_PKG_VERSION"))?;
        write!(self, "}}")?; // meta
        write!(self, "}}")?; // project
        Ok(())
    }
    ...
    pub fn sprite(
        ...
        if name == STAGE_NAME {
            ...
            write!(self, r#","text":{}"#, json!(TurbowarpConfig::from(config).to_string()))?;
            write!(self, "}}")?; // twconfig
            write!(self, "}}")?; // comments
        }
        ...
        for proc in sprite.procs.values() {
            self.proc(S { stage, sprite, proc: Some(proc) }, d, proc)?;
        }
        for event in &sprite.events {
            self.event(S { stage, sprite, proc: None }, d, event)?;
        }
        write!(self, "}}")?; // blocks
    ...
    pub fn var_declaration(&mut self, sprite: &Sprite, var: &Var, comma: &mut bool, d: D) -> io::Result<()> {
        match &var.type_ {
            Type::Value => {
                self.json_var_declaration(&var.name, comma)?;
            }
            Type::Struct { name: type_name, span: type_span } => {
                let Some(struct_) = sprite.structs.get(type_name) else {
                    d.report(DiagnosticKind::UnrecognizedStruct(type_name.clone()), type_span);
                    return Ok(());
                };
    ...
    pub fn local_var_declaration(&mut self, sprite: &Sprite, proc: &Proc, var: &Var, comma: &mut bool, d: D) -> io::Result<()> {
        match &var.type_ {
            Type::Value => {
            ...
            Type::Struct { name: type_name, span: type_span } => {
                ...
                for field in &struct_.fields {
                    let qualified_var_name =
                        qualify_local_var_name(&proc.name, &qualify_struct_var_name(&field.name, &var.name));
                    self.json_var_declaration(&qualified_var_name, comma)?;
                }
            }
        }
    ...
    pub fn list_declaration(&mut self, input: &Path, sprite: &Sprite, list: &List, comma: &mut bool, d: D) -> io::Result<()> {
        let data = match &list.cmd {
            Some(cmd) => {
                eprintln!("{cmd:#?}");
                let output =
                    Command::new("/usr/bin/sh").arg("-c").arg(format!("cd {};{}", input.display(), cmd.cmd)).output()?;
                if !output.status.success() {
                    d.report(DiagnosticKind::CommandFailed { stderr: output.stderr }, &cmd.span);
                }
                ...
                for (i, field) in struct_.fields.iter().enumerate() {
                    ...
                    if let Some(cmd) = &data {
                        let column = (0..(cmd.len() / struct_.fields.len()))
                            ...
                            .collect::<Vec<_>>();
                        write!(self, r#""{}":["{}",{}]"#, qualified_list_name, qualified_list_name, json!(column))?;
                    } else {
                        write!(self, r#""{}":["{}",[]]"#, qualified_list_name, qualified_list_name)?;
                    }
                }
            }
        }
    ...
    pub fn costume(&mut self, input: &Path, costume: &Costume, d: D) -> io::Result<()> {
        ...
        let hash = self.costumes.get(&costume.path).cloned().map(Ok::<_, io::Error>).unwrap_or_else(|| {
            if !path.is_file() {
                d.report(DiagnosticKind::FileNotFound(costume.path.clone()), &costume.span);
    ...
    pub fn proc(&mut self, s: S, d: D, proc: &Proc) -> io::Result<()> {
        ...
        for arg in &proc.args {
            match &arg.type_ {
                Type::Value => {
                    ...
                    self.begin_node(Node::new("argument_reporter_string_number", arg_id).parent_id(prototype_id).shadow(true))?;
                ...
                Type::Struct { name: type_name, span: type_span } => {
                    ...
                    for field in &struct_.fields {
                        ...
                        let arg_id = self.id.new_id();
                        self.begin_node(
                            Node::new("argument_reporter_string_number", arg_id).parent_id(prototype_id).shadow(true),
                        )?;
        ...
        self.begin_node(Node::new("procedures_prototype", prototype_id).parent_id(this_id).shadow(true))?;
        self.begin_inputs()?;
        let mut comma = false;
        ...
        self.end_obj()?; // inputs
        write!(self, "{}", Mutation::prototype(proc.name.clone(), &qualified_args, proc.warp,))?;
        self.end_obj()?; // node
    ...
    pub fn event(&mut self, s: S, d: D, event: &Event) -> io::Result<()> {
        ...
        let next_id = self.id.new_id();
        self.begin_node(
            Node::new(event.kind.opcode(), this_id).some_next_id((!event.body.is_empty()).then_some(next_id)).top_level(true),
        )?;
        match &event.kind {
            ...
            EventKind::OnBackdrop { backdrop, span } => self.on_backdrop(s, d, this_id, backdrop, span),
    ...
    pub fn stmts(&mut self, s: S, d: D, stmts: &[Stmt], mut this_id: NodeID, mut parent_id: Option<NodeID>) -> io::Result<()> {
    ...
    pub fn stmt(
        ...
        self.begin_node(Node::new(stmt.opcode(s), this_id).some_next_id(next_id).some_parent_id(parent_id))?;
        match stmt {
            Stmt::Repeat { times, body } => self.repeat(s, d, this_id, times, body),
            Stmt::Forever { body, span } => self.forever(s, d, this_id, body, span),
            Stmt::Branch { cond, if_body, else_body } => self.branch(s, d, this_id, cond, if_body, else_body),
            Stmt::Until { cond, body } => self.until(s, d, this_id, cond, body),
            Stmt::SetVar { name, value, type_, is_local } => self.set_var(s, d, this_id, name, value, type_, is_local),
            ...
            Stmt::DeleteListIndex { name, index } => self.delete_list_index(s, d, this_id, name, index),
            Stmt::DeleteList(name) => self.delete_list(s, d, name),
            Stmt::InsertAtList { name, index, value } => self.list_insert(s, d, this_id, name, index, value),
            Stmt::SetListIndex { name, index, value } => self.set_list_index(s, d, this_id, name, index, value),
    ...
    pub fn expr(&mut self, s: S, d: D, expr: &Expr, this_id: NodeID, parent_id: NodeID) -> io::Result<()> {
        match expr {
            Expr::Value { .. } => Ok(()),
            Expr::Name { .. } => Ok(()),
            Expr::Arg(name) => self.arg(s, d, this_id, parent_id, name),
            Expr::Repr { repr, span, args } => self.repr(s, d, this_id, parent_id, repr, span, args),
            Expr::UnOp { op, span, opr } => self.un_op(s, d, this_id, parent_id, op, span, opr),
            Expr::BinOp { op, span, lhs, rhs } => self.bin_op(s, d, this_id, parent_id, op, span, lhs, rhs),
            Expr::StructLiteral { name, span, .. } => {
                d.report(
                    DiagnosticKind::TypeMismatch {
                        expected: Type::Value,
                        given: Type::Struct { name: name.clone(), span: span.clone() },
                    },
                    &expr.span(),
                );
                Ok(())
            }
            Expr::Dot { .. } => panic!("attempted to codegen dot"),
        }
    }
}
