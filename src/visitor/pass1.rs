...
impl<'a> S<'a> {
    fn get_var(&self, name: &str) -> Option<&Var> {
        self.local_vars
            ...
            .or_else(|| self.global_vars.and_then(|global_vars| global_vars.get(name)))
    }

    fn get_list(&self, name: &str) -> Option<&List> {
        self.lists.get(name).or_else(|| self.global_lists.and_then(|global_lists| global_lists.get(name)))
    }

    fn get_struct(&self, name: &str) -> Option<&Struct> {
        self.structs.get(name).or_else(|| self.global_structs.and_then(|global_structs| global_structs.get(name)))
    }
}
...
pub fn visit_project(
    ...
    for (sprite_name, sprite) in &mut project.sprites {
        visit_sprite(sprite, Some(&project.stage), sprites_diagnostics.get_mut(sprite_name).unwrap());
    }
}
...
fn visit_stmts(stmts: &mut Vec<Stmt>, s: S, d: D) {
    ...
    while i < stmts.len() {
        let replace = match &stmts[i] {
            Stmt::SetVar { name, value, type_, is_local } => visit_stmt_set_var(s, d, name, value, type_, is_local),
            Stmt::SetListIndex { name, index, value } => visit_stmt_list_set(s, d, name, index, value),
            Stmt::AddToList { name, value } => visit_stmt_list_add(s, d, name, value),
            Stmt::DeleteList(name) => visit_stmt_delete_list(s, d, name),
            Stmt::DeleteListIndex { name, index } => visit_stmt_delete_list_index(s, d, name, index),
            Stmt::InsertAtList { name, index, value } => visit_stmt_insert_at_list(s, d, name, index, value),
            _ => None,
        };
...
fn visit_stmt(stmt: &mut Stmt, s: S, d: D) {
    match stmt {
        ...
        Stmt::SetVar { name: _, value, type_: _, is_local: _ } => {
        ...
        Stmt::InsertAtList { name: _, index, value } => {
            visit_expr(value, s, d);
            visit_expr(index, s, d);
        }
        Stmt::SetListIndex { name: _, index, value } => {
            visit_expr(value, s, d);
            visit_expr(index, s, d);
        }
        ...
        Stmt::ProcCall { name: _, span: _, args } => {
            for arg in args {
                visit_expr(arg, s, d);
            }
        }
...
fn visit_expr(expr: &mut Rrc<Expr>, s: S, d: D) {
    let replace: Option<Rrc<Expr>> = match &mut *expr.borrow_mut() {
        ...
        Expr::Repr { repr: _, span: _, args } => {
            for arg in args {
                visit_expr(arg, s, d);
            }
            None
        }
        Expr::UnOp { op: _, span: _, opr } => {
            visit_expr(opr, s, d);
            None
        }
        Expr::BinOp { op, span, lhs, rhs } => {
            ...
            match op {
                ...
                BinOp::Le => Some(
                    UnOp::Not.to_expr(span.clone(), BinOp::Gt.to_expr(span.clone(), lhs.clone(), rhs.clone()).into()).into(),
                ),
                BinOp::Ge => Some(
                    UnOp::Not.to_expr(span.clone(), BinOp::Lt.to_expr(span.clone(), lhs.clone(), rhs.clone()).into()).into(),
                ),
                BinOp::Ne => Some(
                    UnOp::Not.to_expr(span.clone(), BinOp::Eq.to_expr(span.clone(), lhs.clone(), rhs.clone()).into()).into(),
                ),
                BinOp::FloorDiv => Some(
                    UnOp::Floor.to_expr(span.clone(), BinOp::Div.to_expr(span.clone(), lhs.clone(), rhs.clone()).into()).into(),
                ),
            }
        }
        Expr::StructLiteral { name: _, span: _, fields } => {
            for field in fields {
...
fn visit_expr_bin_op_of(s: S, span: &Span, lhs: &Rrc<Expr>, rhs: &Rrc<Expr>) -> Option<Rrc<Expr>> {
    let Expr::Name(Name::Name { name, span }) = &*lhs.borrow() else { return None };
...
fn visit_expr_dot(d: D, lhs: &Rrc<Expr>, rhs: &SmolStr, rhs_span: &Span) -> Option<Rrc<Expr>> {
    let Expr::StructLiteral { name: lhs_name, span: _, fields } = &*lhs.borrow() else { return None };
    let Some(field) = fields.iter().find(|field| &field.name == rhs) else {
        d.report(DiagnosticKind::StructDoesNotHaveField { type_name: lhs_name.clone(), field_name: rhs.clone() }, rhs_span);
        return None;
    };
    Some(field.value.clone())
}

fn visit_stmt_set_var(s: S, d: D, name: &Name, value: &Rrc<Expr>, _type: &Type, _is_local: &bool) -> Option<Vec<Stmt>> {
    let expr = &*value.borrow();
    let struct_literal_fields =
        get_struct_literal_for_type(s, d, name, expr, |basename| s.get_var(basename).map(|var| &var.type_))?;
    Some(
...
fn visit_stmt_list_set(s: S, d: D, name: &Name, index: &Rrc<Expr>, value: &Rrc<Expr>) -> Option<Vec<Stmt>> {
    let expr = &*value.borrow();
    let struct_literal_fields =
        get_struct_literal_for_type(s, d, name, expr, |basename| s.get_list(basename).map(|list| &list.type_))?;
    Some(
        struct_literal_fields
            .iter()
...
fn visit_stmt_insert_at_list(s: S, d: D, name: &Name, index: &Rrc<Expr>, value: &Rrc<Expr>) -> Option<Vec<Stmt>> {
    let expr = &*value.borrow();
    let struct_literal_fields =
        get_struct_literal_for_type(s, d, name, expr, |basename| s.get_list(basename).map(|list| &list.type_))?;
    Some(
    ...
    T: FnOnce(&str) -> Option<&'a Type>,
{
    ...
    let struct_ = s.get_struct(type_name)?;
    let Expr::StructLiteral { name: struct_literal_name, span: struct_literal_span, fields: struct_literal_fields } = expr
    else {
        d.report(DiagnosticKind::TypeMismatch { expected: type_.clone(), given: Type::Value }, &basespan);
        return None;
    };
    let Some(value_struct) = s.get_struct(struct_literal_name) else {
        d.report(DiagnosticKind::UnrecognizedStruct(struct_literal_name.clone()), struct_literal_span);
        return None;
    };
    if struct_.name != value_struct.name {
        d.report(
            DiagnosticKind::TypeMismatch {
                expected: Type::Struct { name: struct_.name.clone(), span: type_span.clone() },
                given: Type::Struct { name: value_struct.name.clone(), span: struct_literal_span.clone() },
            },
            &basespan,
        );
        return None;
