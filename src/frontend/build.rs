...
pub fn build(input: Option<PathBuf>, output: Option<PathBuf>) -> Result<(), BuildError> {
    ...
    let config: Config = toml::from_str(&config_src).with_context(|| format!("failed to parse {}", config_path.display()))?;
    ...
    for sprite_path in fs::read_dir(&input)? {
        ...
        if !sprite_path.extension().is_some_and(|extension| extension == "gs") {
            continue;
        }
        if !sprite_path.is_file() {
            continue;
        }
        let sprite_name: SmolStr = sprite_path.file_stem().unwrap_or_default().to_str().unwrap().into();
        let mut sprite_diagnostics = SpriteDiagnostics::new(sprite_path)?;
        let sprite =
            parser::parse(&sprite_diagnostics.src).map_err(|err| sprite_diagnostics.diagnostics.push(err)).unwrap_or_default();
    ...
    if !(stage_diagnostics.diagnostics.is_empty()
        && sprites_diagnostics.values().all(|sprite_diagnostics| sprite_diagnostics.diagnostics.is_empty()))
    {
        return Err(ProjectDiagnostics { project, stage_diagnostics, sprites_diagnostics }.into());
    }
    visitor::pass0::visit_project(&mut project);
    visitor::pass1::visit_project(&mut project, &mut stage_diagnostics, &mut sprites_diagnostics);
    println!("{:#?}", project);
    let mut sb3 = Sb3::new(BufWriter::new(File::create(output)?));
    sb3.project(&input, &project, &config, &mut stage_diagnostics, &mut sprites_diagnostics)?;
    if !(stage_diagnostics.diagnostics.is_empty()
        && sprites_diagnostics.values().all(|sprite_diagnostics| sprite_diagnostics.diagnostics.is_empty()))
    {
        return Err(ProjectDiagnostics { project, stage_diagnostics, sprites_diagnostics }.into());
