...
pub enum Expr {
    ...
    Name(Name),
    Dot { lhs: Rrc<Expr>, rhs: SmolStr, rhs_span: Span },
    Arg(Name),
    Repr { repr: Repr, span: Span, args: Vec<Rrc<Expr>> },
    UnOp { op: UnOp, span: Span, opr: Rrc<Expr> },
    BinOp { op: BinOp, span: Span, lhs: Rrc<Expr>, rhs: Rrc<Expr> },
    StructLiteral { name: SmolStr, span: Span, fields: Vec<StructLiteralField> },
}
...
impl UnOp {
    pub fn to_expr(self, span: Span, expr: Rrc<Expr>) -> Expr {
        Expr::UnOp { op: self, span, opr: expr }
    }
}

impl BinOp {
    pub fn to_expr(self, span: Span, lhs: Rrc<Expr>, rhs: Rrc<Expr>) -> Expr {
        Expr::BinOp { op: self, span, lhs, rhs }
    }
}
