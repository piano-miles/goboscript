...
impl DiagnosticKind {
    pub fn to_string(&self, project: &Project, sprite_diagnostics: &SpriteDiagnostics) -> String {
        match self {
            ...
            DiagnosticKind::UnrecognizedEof(vec) => format!("unrecognized eof, expected one of {:?}", vec),
            ...
            DiagnosticKind::UnrecognizedStructField(name) => format!("unrecognized struct field {name}"),
            DiagnosticKind::UnrecognizedEnumVariant(name) => format!("unrecognized enum variant {name}"),
            ...
            DiagnosticKind::BlockArgsCountMismatch { block, given } => {
                format!("block {:?} expects {} arguments, but {} were given", block, block.args().len(), given)
            }
            DiagnosticKind::ReprArgsCountMismatch { repr, given } => {
                format!("repr {:?} expects {} arguments, but {} were given", repr, repr.args().len(), given)
            }
            DiagnosticKind::ProcArgsCountMismatch { proc, given } => {
                format!("proc {:?} expects unknown arguments, but {} were given", proc, given)
            }
            ...
            DiagnosticKind::StructDoesNotHaveField { type_name, field_name } => {
